...
public class armMove extends CommandBase {
  ...
  public void end(boolean interrupted) {
    ...
    if(state == ArmLift.positionStates.REST){
      RobotContainer.m_armLift.movePosFF(Constants.REST);
    }
    else if(state == ArmLift.positionStates.FLOOR){
      RobotContainer.m_armLift.movePosFF(Constants.FLOOR);
    }
    else if(state == ArmLift.positionStates.CUBE){
      RobotContainer.m_armLift.movePosFF(Constants.CUBE);
    }
    else if(state == ArmLift.positionStates.CONE){
      RobotContainer.m_armLift.movePosFF(Constants.CONE);
    }
    else if(state == ArmLift.positionStates.PICKUP){
      RobotContainer.m_armLift.movePosFF(Constants.PICKUP);
    }
    else if(state == ArmLift.positionStates.CHARGING_STATION){
      RobotContainer.m_armLift.movePosFF(Constants.CHARGING_STATION);
    }
  }
